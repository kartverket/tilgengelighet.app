...
export default function ObjectEditor(props: ObjectEditorProps) {
  ...
  const [uneditedFeatureMember, setUneditedFeatureMember] = useState<
    FeatureMember | undefined
  >(() => {
    if (props.featureMember) {
      const feature = getParsedFeatureMember(
        props.featureMember.toLocalStorageFormatted(),
      );

      const initImages = props.featureMember.images;
      if (initImages.length > 0) {
        for (var element of initImages) {
          ...
          if (isAlreadyAdded == false) feature.images.push(element);
        }
      }
      ...
      if (objectType.hasRampFields) {
        ...
        const children = rampType?.elements.map(
          element =>
            new FeatureNode(
              element.name,
              props.objectProps.copy?.rampNode?.nodes.find(
        ...
        if (children![0].value != 'Ja') children![0].value = 'Nei';
      ...
      if (objectType.hasStairFields) {
        ...
        const children = stairType?.elements.map(
          element =>
            new FeatureNode(
              element.name,
              props.objectProps.copy?.stairNode?.nodes.find(
                e => e.name.toLowerCase() == element.name.toLowerCase(),
              )?.value,
              true,
              element.type as XsdPrimitiveType,
      ...
      if (
        ...
        if (node.dependency != null) {
          const dependsOnValue = (
            node.dependency?.dependsOnNode ??
            fields.find(n => n.name == node.dependency?.dependsOnNodeName)
          )?.value?.toLowerCase();

          if (node.dependency.dependsOnValue.includes(',')) {
            const values = node.dependency.dependsOnValue
              .toLowerCase()
              .split(',');

            if (!values.includes(dependsOnValue)) {
              node.value == undefined;

              continue;
            }
          } else if (
            dependsOnValue != node.dependency.dependsOnValue?.toLowerCase()
          ) {
            node.value == undefined;
            continue;
          }
  ...
  const onApproveRegistration = async () => {
    if (validate() == true) {
      ...
      onRebuildObjectLayer!({
        id: featureMember.localId ?? 'substitute',
    ...
    <div className={classes.center}>
      <TopBar
        onPop={() => {
          ...
          if (
            hasMadeChange ||
            uneditedFeatureMember?.images.length !== featureMember.images.length
          ) {
            saveData!(uneditedFeatureMember);
          }
        }}

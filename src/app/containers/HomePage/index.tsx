...
export type ContextProps = {
  ...
  onSetConnectionErrorMessage?: () => void;
};
...
export function HomePage() {
  ...
  const [showObjectsByColorType, setShowObjectsByColorType] = useState<
    ShowObjectsByColorType
  >(ShowObjectsByColorType.registration);
  ...
  const [data, setData] = useState<ContextProps>({
    ...
    onSetConnectionErrorMessage: () => setConnectionErrorMessage(),
  });
  ...
  const handleOnCreateFeatureMember = async (featureMember: FeatureMember) => {
    ...
    if (data.featureCollection != undefined) {
      ...
      setData({
        ...data,
        featureCollection: data.featureCollection,
        showObjectsByColorType: data.showObjectsByColorType,
      });
    } else {
  ...
  useEffect(() => {
    ...
    if (
      ...
      data.hasUploadConnectionError == undefined
    ) {
  ...
  const handleUploadConnectionError = async () => {
    ...
    if (error == true) {
      if (data.featureCollection?.featureMembers != undefined) {
        ...
        if (featuresToUpload.length > 0) {
          ...
          var uploadedFeatures = 0;

          for (const feature of featuresToUpload) {
            const response = await createFeatureMember(feature);
            ...
            setData({
              ...
              rebuildObjectLayerData: {
                id: feature.localId!,
                updateAction: feature.dbAction,
              },
            });
          }
          if (uploadedFeatures === featuresToUpload.length) {
            LocalStorageProvider.setConnectionError(false);
          }

          setConnectionErrorMsg(
            'Fullført!' + ` ${uploadedFeatures} objekter har blitt lastet opp`,
          );

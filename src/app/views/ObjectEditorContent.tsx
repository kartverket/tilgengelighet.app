...
export function createFormSteps(
  ...
  if (objectType.hasRampFields) {
    ...
    const rampNodes = featureMember.rampNode.nodes;
    const orderedNodes: FeatureNode[] = [];

    for (var i = 0; i < rampType!.elements.length; i++) {
      const element = rampType?.elements[i];

      const node = rampNodes.find(
    ...
    featureMember.rampNode.nodes = orderedNodes;
  ...
  if (objectType.hasStairFields) {
    ...
    for (const element of stairType?.elements!) {
      ...
      if (element.name?.toLowerCase() == 'rekkverkhøydenedre') {
        let required;

        const type = featureMember.type!.toLowerCase();

        if (type == 'tettstedvei' || type == 'tettstedinngangbygg') {
          required = false;
        }
      } else {
        required = getRequiredForFeatureNode('trapp', element.name);
      }
      ...
      if (correspondingNode == undefined) {
        ...
        correspondingNode.required = required;
      }
    }
    ...
    /// Assert correct ordering
    const stairNodes = featureMember.stairNode.nodes;
    const orderedNodes: FeatureNode[] = [];

    for (var i = 0; i < stairType!.elements.length; i++) {
      const element = stairType?.elements[i];

      const node = stairNodes.find(
        n => n?.name?.toLowerCase() == element?.name?.toLowerCase(),
      )!;
    ...
    featureMember.stairNode.nodes = orderedNodes;

...
import { genId, getCenterOfPolygon } from 'app/components/ActualMap';
...
export class FeatureMember {
  ...
  toUploadFormatted(): any[] | object {
    ...
    if (this.geometry?.type.toLowerCase() == 'polygon') {
      ...
      if (
        (this.geometry.coordinates as PolygonLayerCoordinates)
          .representationPoint == undefined
      ) {
        repPoint = getCenterOfPolygon(this.geometry.coordinates);
      } else {
      ...
      // this.geometry.coordinates = (this.geometry
      //   .coordinates as PolygonLayerCoordinates).coordinates;

      // this.geometry.coordinates = coordinates;

      // TODO: Handle erase polygon type.
      // if (this.dbAction != UpdateAction.erase)
      borderValues.set('exterior', [borderObject.id]);

      // if (this.dbAction == UpdateAction.replace) {
      ...
      // if (this.dbAction == UpdateAction.erase) return formattedPolygon;
      // else
  ...
  toLocalStorageFormatted(asFilter?: boolean) {
    ...
    if (this.images != undefined) {
      for (const element of this.images) {
        if (element.isUploaded == false) continue;
  ...
  getDateUploadFormatted(date?: Date, hourIncrement?: number): string {

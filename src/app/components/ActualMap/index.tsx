...
export default React.forwardRef((props: Props, mapRef: any) => {
  ...
  const getLayerCoordinates = ():
    ...
    switch (selectedLayer.pm._shape) {
      ...
      case 'Polygon': {
        ...
        const center = getCenterOfPolygon(latLngs);

        return {
          coordinates: latLngs,
          representationPoint: [center[0], center[1]],
        };
      }
    }
  ...
  React.useEffect(() => {
  let storedtooltipState = JSON.parse(localStorage.getItem('all_tooltips') as string)
  ...
  const getLayerColor = (feature: any): string | undefined => {
    if (showObjectsByColorType == ShowObjectsByColorType.registration) {
    ...
    } else if (showObjectsByColorType == ShowObjectsByColorType.accessibility) {
  ...
  React.useEffect(() => {
    if (selectedLayer && rebuildObjectLayerData?.id) {
      ...
      DrawUtil.deleteLayer(mapRef, oldLayer);

      selectedLayer.addTo(features);
      if (rebuildObjectLayerData.updateAction == UpdateAction.replace) {
        setSelectedLayerState(selectedLayer);
        addHighlightToLayer(selectedLayer);
      } else {
        selectedLayer = undefined;
      }

      // mapRef.current.leafletElement.addLayer(selectedLayer);
    }
  }, [rebuildObjectLayerData]);
        ...
        <div
          ...
          <EditToolbar
            ...
            onDelete={() => {
              ...
              if (featureMember == undefined)
                throw 'FeatureMember cannot be undefined';
...
export const getCenterOfPolygon = (coordinates: LatLng[]) => {
